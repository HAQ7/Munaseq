...
export default function forwhoForm({
  ...
  const [modalType, setModalType] = useState<"presenter" | "moderator">(
    "presenter"
  );
  ...
  return (
    <CreateEventCard actual={step} goal={4}>
      ...
      {/* Presenters Section */}
      <div className="mt-5">
        <label className="font-medium text-lg">مقدمين الفعالية التعليمية</label>
        <motion.div layout className="flex gap-2 mt-2">
          ...
          {presenters.length < 3 && (
            <motion.div
              ...
              onClick={() => {
                setModalType("presenter");
                setIsModalOpen(true);
              }}
            >
              <Image src={plusIcon} alt="Add presenter" />
            </motion.div>
          )}
        </motion.div>
      </div>
      ...
      {/* Moderators Section */}
      <div className="mt-5">
        <label className="font-medium text-lg">منظمين الفعالية التعليمية</label>
        <motion.div layout className="flex gap-2 mt-2">
          ...
          {moderators.length < 3 && (
            ...
            >
              <Image src={plusIcon} alt="Add moderator" />
            </motion.div>
          )}
        </motion.div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-5 space-y-5 w-96">
            <h2 className="text-xl font-bold">
              {modalType === "presenter" ? "إضافة مقدم" : "إضافة منظم"}
            </h2>
            <input
              type="text"
              value={username}
